...
class FPQSeq2Q(nn.Module):
    ...
    def __init__(self,  n_channels: int =1, 
        ...
        self.tau_proposal = NQuantileProposal(N=N, z_dim=latent_size*2, M=out_size, tau=1)
        self.tau_cosine   = NCosineTau(num_cosines=num_cosines, z_dim=latent_size, M=out_size, activation=activation)
        self.N = N
        self.context_size = context_size
        self.quantile_net = nn.Sequential(FeedForward(latent_size*2, expansion_factor=1, dropout=dropout/2, activation=self.activation), 
                                          self.activation, nn.Linear(latent_size*2, out_size))
                                     
        
        
        self.multihead_attention = nn.MultiheadAttention(latent_size, num_head, dropout=dropout)
        self.dropout = nn.Dropout(dropout)
        self.feats = create_linear(latent_size*24*2,latent_size*2)
    ...
    def get_quantile_values(self, tau, h,z, attn_emb):
        ...
        #attn_emb = F.adaptive_avg_pool2d(q_m_emb, (self.filter_size, self.filter_size))
         
        attn_emb = torch.nn.functional.silu(self.feats(q_m_emb.flatten(2,3)))
        out = self.quantile_net(attn_emb)
    ...
    def get_quantile_proposals(self, x, z, h):
        ...
        attn_emb = torch.nn.functional.silu(self.feats(attn_output.flatten(1,2)))
        taus, tau_hats, entropies =  self.tau_proposal(attn_emb)
